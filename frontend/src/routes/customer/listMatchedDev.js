...
class ListMatchedDevView extends Component {
  ...
  componentDidMount() {
    ...
    if(this.props.role_type_id !== 4) {
      ...
      this.getMatchDeveloperList();
  }

  getMatchDeveloperList() {
    var body = {
      action: 'getMatchedList',
      customerId: this.props.userId
    }
    var self = this;
    axios.post(routeNames.API_PM, body)
    .then(function(response) {
      var data = response.data;
      if(data.success)
      {
        /* WASSIM NEED TO CHANGE THAT TO result */
        self.setState({ developers: data.list });
      }
    })
    .catch(function(_err){
      ...
      <div className='container h-100'>
        <div className='row h-100 w-100'>
          ...
          <div className='col-sm-12 col-md-9 h-100 bg-white setupView'>
            ...
            <div className='row subTitle'><h5>Your Matched developer list</h5></div>
...
export default withRouter(connect(mapStateToProps)(ListMatchedDevView));
